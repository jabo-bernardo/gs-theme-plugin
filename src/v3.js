	...
	class ThemeCustomization {
		...
		async createGUI() {
			...
			const content = `
				...
				<p class="gstc-table-of-contents" style="display: none; height: auto; font-size: 0.8rem">${componentTree.map(component => `<a class="noLoader" href="${currentPath}#${component.name.replace(/ +/g, "-")}" style="font-size: 1rem">${component.name}</a>`).join(' • ')}</p>			
